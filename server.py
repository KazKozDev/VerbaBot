...
class CalendarManager:
    ...
    async def get_google_auth(self):
        """Retrieve Google calendar authentication data"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                db.row_factory = sqlite3.Row
                async with db.execute('SELECT * FROM google_calendar_auth ORDER BY id DESC LIMIT 1') as cursor:
                    row = await cursor.fetchone()
...
class EnhancedRAG:
    ...
    def generate(self, query, chat_history=None, model=None, use_rag=True, system_prompt=None):
        """Generate a response to a query, using RAG if enabled"""
        try:
            if use_rag:
                # Retrieve relevant documents - увеличим количество извлекаемых документов с 5 до 10
...
Using the context provided, answer the human's question. Always refer to the document content when answering, even if the match isn't perfect. If no exact information is found, try to extract relevant details from the document content that might help answer the question. If you are absolutely certain that the context doesn't contain relevant information, only then say you don't have enough information about this specific topic."""
                else:
                    prompt = f"""{system_prompt or "You are a helpful assistant that answers based on the provided context."}
...
Using the context provided, answer the human's question. Always refer to the document content when answering, even if the match isn't perfect. If no exact information is found, try to extract relevant details from the document content that might help answer the question. If you are absolutely certain that the context doesn't contain relevant information, only then say you don't have enough information about this specific topic."""
                ...
                # If RAG is disabled, create a prompt with system prompt and history
                if chat_history and len(chat_history) > 0:
                    history_parts = []
                    for entry in chat_history:
                        role = "Human" if entry.get("is_user") else "Assistant"
...
# Declare global variables to be initialized in init_app
rag_system = None
calendar_rag = None
async def init_db():
    async with aiosqlite.connect('db/assistant.db') as db:
...
async def init_app():
    ...
    try:
        global rag_system, calendar_rag
