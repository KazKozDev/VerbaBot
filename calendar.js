...
class CalendarModule {
    ...
    isCalendarCommand(message) {
        const calendarKeywords = [
            ...
            'tomorrow', 'next week', 'sync calendar', 'reminder',
    ...
    async processCalendarCommand(message) {
        try {
            ...
            // Instead of processing the command locally, send it to the server
            // which has more sophisticated NLP capabilities
            const response = await fetch('http://127.0.0.1:5001/calendar/natural', {
                ...
                body: JSON.stringify({
                    message: message,
                    chat_id: window.currentChatId, // Include the current chat ID
            ...
            if (data.success) {
                ...
                // If the server couldn't process it, show the error message
                this.addMessageToChat(data.message || "I couldn't process your calendar request.", false);
                return false;
            }
    ...
    extractEventInfo(message) {
        ...
        try {
            ...
            // Ищем название события (обычно это текст перед словами "в", "на", "завтра", "сегодня" и т.д.)
            // Но сначала уберем команду "добавь событие" или "добавить событие"
            let cleanMessage = message.replace(/добав(ь|ить)\s+событие\s+/i, '');
            
            // Извлекаем название события
            // Ищем до первого временного маркера
            const titleMatch = cleanMessage.match(/^(.*?)(?:\s+(?:в|на|завтра|сегодня|послезавтра|\d{1,2}[:.]\d{2}|\d{1,2}\s+\w+))/i);
            if (titleMatch && titleMatch[1]) {
                eventInfo.title = titleMatch[1].trim();
            } else {
                // Если не найдено, берем весь текст до первой цифры или даты
                const altTitleMatch = cleanMessage.match(/^(.*?)(?:\s+\d)/i);
                if (altTitleMatch && altTitleMatch[1]) {
                    eventInfo.title = altTitleMatch[1].trim();
                } else {
                    // Последний вариант - просто берем первое слово
            ...
            if (timeMatch) {
                // Извлекаем часы и минуты начала события
                const startHours = parseInt(timeMatch[1]);
                const startMinutes = parseInt(timeMatch[2]);
                
                // Если указана дата (завтра, послезавтра и т.д.)
                if (/завтра/i.test(message)) {
                    startDate.setDate(startDate.getDate() + 1);
                } else if (/послезавтра/i.test(message)) {
                    startDate.setDate(startDate.getDate() + 2);
                } else if (/через (\d+) дн/i.test(message)) {
                    const daysMatch = message.match(/через (\d+) дн/i);
                    if (daysMatch && daysMatch[1]) {
                        startDate.setDate(startDate.getDate() + parseInt(daysMatch[1]));
                    }
                } else if (/(\d{1,2})(?:\s+|\.|\/)(\d{1,2})(?:\s+|\.|\/)(\d{2,4})?/i.test(message)) {
                    // Формат DD.MM.YYYY или DD MM YYYY
                    const dateMatch = message.match(/(\d{1,2})(?:\s+|\.|\/)(\d{1,2})(?:\s+|\.|\/)(\d{2,4})?/i);
                    if (dateMatch) {
                        const day = parseInt(dateMatch[1]);
                        const month = parseInt(dateMatch[2]) - 1; // Месяцы в JS начинаются с 0
                        const year = dateMatch[3] ? (dateMatch[3].length === 2 ? 2000 + parseInt(dateMatch[3]) : parseInt(dateMatch[3])) : startDate.getFullYear();
                        
                        startDate.setFullYear(year, month, day);
                    }
                } else if (/(\d{1,2})\s+(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)/i.test(message)) {
                    // Формат DD Месяц
                    const dateMatch = message.match(/(\d{1,2})\s+(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)/i);
                    if (dateMatch) {
                        const day = parseInt(dateMatch[1]);
                        const months = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря'];
                        const month = months.findIndex(m => m.toLowerCase() === dateMatch[2].toLowerCase());
                ...
                eventInfo.startTime = new Date(startDate);
                
                // Если указано время окончания
                if (timeMatch[3] && timeMatch[4]) {
                    const endDate = new Date(startDate);
                    endDate.setHours(parseInt(timeMatch[3]), parseInt(timeMatch[4]), 0, 0);
                    
                    // Если конечное время меньше начального, предполагаем следующий день
                    ...
                    // По умолчанию продолжительность 1 час
                    const endDate = new Date(startDate);
                    endDate.setHours(endDate.getHours() + 1);
                    eventInfo.endTime = endDate;
                }
            } else {
                // Если конкретное время не указано, но есть дата
                if (/завтра/i.test(message)) {
                    startDate.setDate(startDate.getDate() + 1);
                } else if (/послезавтра/i.test(message)) {
                    startDate.setDate(startDate.getDate() + 2);
            ...
            // Ищем описание (всё после времени)
            const descMatch = message.match(/описание\s*[:]\s*(.+?)(?:\s+место|\s+категория|$)/i);
            if (descMatch && descMatch[1]) {
                eventInfo.description = descMatch[1].trim();
            }
            
            // Ищем место встречи
            const locMatch = message.match(/место\s*[:]\s*(.+?)(?:\s+описание|\s+категория|$)/i);
            if (locMatch && locMatch[1]) {
                eventInfo.location = locMatch[1].trim();
            }
            
            // Ищем категорию
            const catMatch = message.match(/категория\s*[:]\s*(\w+)/i);
            if (catMatch && catMatch[1]) {
                const category = catMatch[1].trim().toLowerCase();
                // Проверяем, существует ли такая категория
                if (['work', 'personal', 'holiday', 'other'].includes(category)) {
